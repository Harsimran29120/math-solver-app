function equationgraphMain() {
    this.version = '9';w = 340; h = 380; graphLt = 0; graphTp = 0; graphWd = 320; graphHt = 236; coords = new Coords(graphLt, graphTp, graphWd, graphHt, -5, -3, 5, 3, true); canvasWd = graphWd; canvasHt = graphHt; clrs = [["black", [0, 0, 125]], ["black", [255, 155, 0]]]; var s = ''; s += '<div style="position:relative; width:' + w + 'px; height:' + h + 'px; border-radius: 15px;  margin:auto; display:block; background-color: white; ">'; inits = ['x*x+y*y-4=0', 'y=x']; for (var i = 0; i < 2; i++) { s += '<input type="text" id="f' + i + '" style="position:absolute; left:10px; top:' + (6 + i * 40) + 'px; color: black; background-color: white; text-align:center; font: bold 20px Arial; color: black; border: 2px black; width:230px; height: 40px ; border-radius: 10px;  z-index: 2;" value="' + inits[i] + '" onkeypress="onKeyPress(event)"/>'; s += '<button style="font: 16px Arial; color: black; border:1px black; position:absolute; height: 40px ; right:9px; top:' + (6 + i * 40) + 'px; z-index:2;" onclick="doGraph()" >Go</button>'; }
